...
# LASSO
fit.pred.lasso <- predict(finalmodel, newx = dat.test.x, type = "response")
results.lasso <- prediction(fit.pred.lasso[,1], dat.test.y)
roc.lasso = performance(results.lasso, measure = "tpr", x.measure = "fpr")

# LDA
pred.lda1 <- predict(fit.lda, newdata = lda.test.x)
results.lda <- prediction(preds1[,2],lda.test.y)
roc.lda = performance(results.lda, measure = "tpr", x.measure = "fpr")

# Random Forest
pred.rf.val1<-predict(train.rf,newdata=dat.val1.rf,type="prob")
results.rf <- prediction(pred.rf.val1[,2], dat.val1.rf$y)
...
legend("bottomright",legend=c("Lasso","Stepwise", "LDA", "RF"),col=c("black","orange","blue","green"),lty=1,lwd=1)
abline(a=0, b= 1)
```
